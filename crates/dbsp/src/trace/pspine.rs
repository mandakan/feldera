...
struct CommittedSpine<B: Batch> {
    ...
    key_filter: Option<B::Key>,
    value_filter: Option<B::Val>,
}
    ...
    fn cursor(&self) -> Self::Cursor<'_> {
        ...
        for merge_state in self.merging.iter().rev() {
            match merge_state {
                ...
                MergeState::Double(MergeVariant::Complete(None, _))
        ...
        F: FnMut(&B),
    {
        for batch in self.merging.iter().rev() {
            match batch {
                ...
                MergeState::Double(MergeVariant::Complete(Some(batch), _)) => map(batch),
        ...
        F: FnMut(T, &B) -> T,
    {
        self.merging
            ...
            .fold(init, |acc, batch| match batch {
                ...
                MergeState::Double(MergeVariant::Complete(Some(batch), _)) => fold(acc, batch),
    ...
    B::Val: Ord,
{
    ...
    fn commit(&self, cid: usize) -> Result<(), Error> {
...
impl<B: Batch> From<&Spine<B>> for CommittedSpine<B> {
    fn from(value: &Spine<B>) -> Self {
        ...
        CommittedSpine {
            ...
            dirty: value.dirty,
            lower_key_bound: value.lower_key_bound.clone(),
            key_filter: None,   // value.key_filter.map(xxx),
            value_filter: None, // value.value_filter.map(xxx),
        }
    }
}
    ...
    B::Val: Ord,
{
    ...
    fn complete_at(&mut self, index: usize) -> Option<B> {
        let (batch, lineage) = self.merging[index].complete(&self.key_filter, &self.value_filter);
    ...
    B: Batch,
{
    ...
    fn complete(
        ...
        key_filter: &Option<Filter<B::Key>>,
        value_filter: &Option<Filter<B::Val>>,
    ) -> (Option<B>, Option<Tup2<PathBuf, PathBuf>>) {
        match replace(self, MergeState::Vacant) {
            MergeState::Vacant => (None, None),
            MergeState::Single(batch) => (batch, None),
    ...
    fn is_complete(&self) -> bool {
        matches!(self, MergeState::Double(MergeVariant::Complete(_, _)))
    }
    ...
    fn begin_merge(batch1: Option<B>, batch2: Option<B>) -> MergeState<B> {
        let variant = match (batch1, batch2) {
            ...
            (batch @ Some(_), None) | (None, batch @ Some(_)) => {
                // This just moves a (single) batch, we don't need to
                // record a lineage as there is no merge.
                MergeVariant::Complete(batch, None)
            }
            (None, None) => MergeVariant::Complete(None, None),
        };
    ...
    B: Batch,
{
    ...
    /// non-trivial batch.
    /// The second option is a pair of persistent ids of the input batches.
    /// This is used to track the lineage of the batch.
    Complete(Option<B>, Option<Tup2<PathBuf, PathBuf>>),
}
    ...
    B: Batch,
{
    ...
    fn complete(
        ...
        if let MergeVariant::Complete(batch, previous_ids) = self {
            (batch, previous_ids)
        } else {
    ...
    fn work(
        ...
        let variant = replace(self, MergeVariant::Complete(None, None));
        if let MergeVariant::InProgress(b1, b2, mut merge) = variant {
            merge.work(&b1, &b2, key_filter, value_filter, fuel);
            if *fuel > 0 {
                assert!(
                    (b1.persistent_id().is_some() && b2.persistent_id().is_some())
                        || (b1.persistent_id().is_none() && b2.persistent_id().is_none()),
                    "persistent id should always set for both batches -- at least for now -- if this no longer holds, adjust the None case below"
                );
                let lineage =
                    if let (Some(id1), Some(id2)) = (b1.persistent_id(), b2.persistent_id()) {
                        Some(Tup2(id1, id2))
                    } else {
                        None
                    };
                *self = MergeVariant::Complete(Some(merge.done()), lineage);
            } else {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InProgress(batch1, batch2, merger) => f
                ...
                .field(merger)
                .finish(),
            Self::Complete(batch, _) => f.debug_tuple("Complete").field(batch).finish(),
        }
    }
}
