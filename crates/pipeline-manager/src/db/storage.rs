...
pub(crate) trait Storage {
    ...
    #[allow(dead_code)] // TODO: currently unused with addition of `clear_ongoing_sql_compilation()`
    ...
    /// and is of the current platform version. Returns `None` if none is found.
    ...
    ///
    /// If the platform version is not the current one, its `platform_version` will be updated and
    /// the `program_status` will be set back to `Pending` such that the SQL compiler can pick it
    /// up again.
    async fn clear_ongoing_rust_compilation(&self, platform_version: &str) -> Result<(), DBError>;

    /// Retrieves the pipeline whose program has been SqlCompiled for the longest
    /// and is of the current platform version. Returns `None` if none is found.
