...
pub(crate) async fn update_pipeline(
    ...
    // Pipeline update is only possible if it is fully shutdown
...
pub(crate) async fn set_program_status(
    ...
    // Pipeline program_status update is only possible if it is fully shutdown
    if current.deployment_status != PipelineStatus::Shutdown
        || current.deployment_desired_status != PipelineDesiredStatus::Shutdown
    {
...
pub(crate) async fn set_deployment_desired_status(
    ...
    // Pipeline desired status set to Running/Paused is only possible
    // if the program is successfully compiled. Note that it does not
    // check whether it is the current platform version, such that
    // already running pipelines can still be resumed/paused. The runner
    // however will error in this case.
    if new_desired_status != PipelineDesiredStatus::Shutdown
        && current.program_status != ProgramStatus::Success
    {
        return Err(DBError::StartFailedDueToProgramNotCompiled);
    }
...
pub(crate) async fn set_deployment_status(
    ...
    // Pipeline deployment status update is only possible
    // if the pipeline program is successfully compiled
...
/// and is of the current platform version.
pub(crate) async fn get_next_sql_compilation(
    ...
    let stmt = txn
        .prepare_cached(&format!(
            "SELECT {RETRIEVE_PIPELINE_COLUMNS}
             FROM pipeline AS p
             WHERE p.program_status = 'pending' AND p.platform_version = $1
...
/// Retrieves the pipeline whose program status has been SqlCompiled for the longest
/// and is of the current platform version.
pub(crate) async fn get_next_rust_compilation(
    ...
    let stmt = txn
        .prepare_cached(&format!(
            "SELECT {RETRIEVE_PIPELINE_COLUMNS}
             FROM pipeline AS p
             WHERE p.program_status = 'sql_compiled' AND p.platform_version = $1
             ORDER BY p.program_status_since ASC, p.id ASC
             LIMIT 1
            "
        ))
