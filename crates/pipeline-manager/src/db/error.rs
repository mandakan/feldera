...
pub enum DBError {
    ...
    StartFailedDueToProgramNotCompiled,
    StartFailedDueToIncorrectPlatformVersion {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DBError::OutdatedProgramVersion {
                ...
                write!(
                    f,
                    "Outdated program version ({outdated_version}). Latest version: {latest_version}"
                )
            }
            DBError::InvalidConnectorTransport { reason } => {
                write!(f, "Invalid connector transport: '{reason}'")
            }
            DBError::StartFailedDueToProgramNotCompiled => {
                write!(
                    f,
                    "Not possible to start the pipeline because the program is not compiled"
                )
            }
            DBError::StartFailedDueToIncorrectPlatformVersion {
                ...
                write!(
                    f,
                    "Unable to start pipeline because it was compiled under a prior platform version ({pipeline_platform_version}) which is not the current ({current_platform_version}).\n\
                    This is due to the Feldera platform having been upgraded inbetween when it was compiled and now.\n\
                    Resolution: (1) shut the pipeline down and (2) make a trivial edit in the SQL to trigger recompilation under the current platform.\n\
                    After the above instructions are followed, it can be again started as usual."
                )
            }
...
impl DetailedError for DBError {
    fn error_code(&self) -> Cow<'static, str> {
        match self {
            ...
            Self::StartFailedDueToProgramNotCompiled { .. } => {
                Cow::from("StartFailedDueToProgramNotCompiled")
            }
            Self::StartFailedDueToIncorrectPlatformVersion { .. } => {
                Cow::from("StartFailedDueToIncorrectPlatformVersion")
            }
...
impl ResponseError for DBError {
    fn status_code(&self) -> StatusCode {
        match self {
            ...
            Self::StartFailedDueToProgramNotCompiled => StatusCode::BAD_REQUEST,
            Self::StartFailedDueToIncorrectPlatformVersion { .. } => {
