...
fn decode_url_encoded_parameter(value: Option<&str>) -> Result<String, ManagerError> {
    match value {
...
async fn get_binary(
    ...
    let pipeline_id = decode_url_encoded_parameter(path_parameters.get("pipeline_id"))?;
    let program_version = decode_url_encoded_parameter(path_parameters.get("program_version"))?;
    let source_checksum = decode_url_encoded_parameter(path_parameters.get("source_checksum"))?;
    let integrity_checksum =
        decode_url_encoded_parameter(path_parameters.get("integrity_checksum"))?;
