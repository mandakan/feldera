...
pub async fn sql_compiler_task(
    ...
    loop {
        ...
        // Compile
        if let Err(e) =
            attempt_end_to_end_sql_compilation(&common_config, &config, db.clone()).await
        {
            match e {
...
///
/// Returns early if:
/// - No pipeline is found for which to compile SQL
        ...
        match exit_status.code() {
            None => {
                Err(SqlCompilationError::SystemError(
                    "No exit status code present because the SQL compiler process was terminated by a signal".to_string()
                ))
            }
            Some(exit_code) => {
                let stderr_str = fs::read_to_string(output_stderr_file_path.clone()).await.map_err(|e| {
                    SqlCompilationError::SystemError(CommonError::io_error(format!("reading stderr file '{}'", output_stderr_file_path.display()), e).to_string())
                })?;
                let messages: serde_json::Result<Vec<SqlCompilerMessage>> = serde_json::from_str(&stderr_str);
                match messages {
                    Ok(messages) => {
