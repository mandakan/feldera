...
const POLL_ERROR_INTERVAL: Duration = Duration::from_millis(30_000);
...
const CLEANUP_INTERVAL: Duration = Duration::from_millis(10_000);
...
pub async fn rust_compiler_task(
    ...
    loop {
        ...
        // Compile
        if let Err(e) =
            attempt_end_to_end_rust_compilation(&common_config, &config, db.clone()).await
        {
            match e {
...
///
/// Returns early if:
/// - No pipeline is found for which to compile Rust
...
async fn call_compiler(
    ...
    if exit_status.success() {
        ...
        match exit_status.code() {
            None => {
                Err(RustCompilationError::SystemError(
                    "No exit status code present because the Rust compiler process was terminated by a signal".to_string()
                ))
            }
            Some(exit_code) => {
                let stdout_str = fs::read_to_string(stdout_file_path.clone()).await.map_err(|e| {
                    RustCompilationError::SystemError(CommonError::io_error(format!("reading file '{}'", stdout_file_path.display()), e).to_string())
                })?;
                let stderr_str = fs::read_to_string(stderr_file_path.clone()).await.map_err(|e| {
                    RustCompilationError::SystemError(CommonError::io_error(format!("reading file '{}'", stderr_file_path.display()), e).to_string())
                })?;
...
async fn cleanup_rust_compilation(
    ...
    for profile in ["dev", "unoptimized", "optimized"] {
        let target_profile_dir = config
            .working_dir()
            .join("rust-compilation")
            .join("target")
            .join(profile);
        if target_profile_dir.is_dir() {
            // target/<profile>: "project-<source-checksum>.<other>" or else "project-<source-checksum>"
            let existing_source_checksums_clone = existing_source_checksums.clone();
            cleanup_specific_files(
                &format!("Rust compilation target {profile}"),
                &target_profile_dir,
            ...
            // target/<profile>/deps: "project_<source-checksum>-<other>"
            let deps_dir = target_profile_dir.join("deps");
            if deps_dir.is_dir() {
                let existing_source_checksums_clone = existing_source_checksums.clone();
                cleanup_specific_files(
                    &format!("Rust compilation target {profile} deps"),
                    &deps_dir,
                    Arc::new(move |name: &str| {
                        decide_cleanup(
                            name,
            ...
            // target/<profile>/.fingerprint: "project-<source-checksum>-<other>"
            let fingerprint_dir = target_profile_dir.join(".fingerprint");
            if fingerprint_dir.is_dir() {
                let existing_source_checksums_clone = existing_source_checksums.clone();
                cleanup_specific_directories(
                    &format!("Rust compilation target {profile} .fingerprint"),
                    &fingerprint_dir,
                    Arc::new(move |name: &str| {
                        decide_cleanup(
                            name,
