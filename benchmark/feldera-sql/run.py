...
def add_connector(connector_name, relation_name, is_input):
    ...
    requests.put(
        f"{api_url}/v0/connectors/{connector_name}", json=json
    ).raise_for_status()
    return {
...
def main():
    ...
    parser.add_argument(
        "--kafka-broker", required=True, help="Kafka broker (e.g., localhost:9092 )"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--merge",
        action=argparse.BooleanOptionalAction,
        help="whether to merge all the queries into one program (default: --no-lateness)",
    )
    parser.add_argument(
        "--storage",
        action=argparse.BooleanOptionalAction,
    ...
    parser.add_argument(
        "--query",
        action="append",
        help="queries to run (by default, all queries), specify one or more of: "
        + ",".join(sort_queries(QUERY_SQL.keys())),
    )
    parser.add_argument(
        "--input-topic-suffix",
    ...
    print(f"4")
    arg0 = parser.parse_args().query
    print(f"4.0")
    arg = parse_queries(arg0)
    print(f"4.1")
    queries = sort_queries(arg)
    print(f"5")
    ...
    for program_name in queries:
        while True:
            status = requests.get(f"{api_url}/v0/programs/{program_name}").json()[
                "status"
            ]
    ...
    input_connectors = [
        add_input_connector(s + suffix, s) for s in ("auction", "bid", "person")
    ]
    output_connectors = {}
    ...
    for program_name in queries:
        ...
        requests.put(
            ...
            json={
                ...
                + [output_connectors[s] for s in program_name.split(",")],
            },
        ).raise_for_status()

    # Stop pipelines
    print(f"Stopping pipeline(s)...")
    ...
    for pipeline_name in queries:
        ...
        while True:
            ...
            if processed > last_processed:
                ...
                sys.stdout.write(
                    f"{before}Pipeline {pipeline_name} processed {processed} records in {elapsed:.1f} seconds{after}"
                )
